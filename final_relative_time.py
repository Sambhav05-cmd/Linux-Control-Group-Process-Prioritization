...
if pid == 0:
    ...
    STATS_STRUCT = struct.Struct("Q Q Q Q Q d")  # ready, blocked, swapped, running, faults,resident page ratio
...
def second_child_fn(child_pid):
    ...
    while True:
        ...
        if stats:
            total_time = stats.total_ready_time + stats.total_blocked_time + stats.total_swapped_time + stats.total_running_time
            data1 = struct.pack("Q Q Q Q", int(stats.total_ready_time/total_time), int(stats.total_blocked_time/total_time),
                                int(stats.total_swapped_time/total_time), int(stats.total_running_time/total_time))
            data2 = struct.pack("Q d", int(faults), vmrss / (vmrss + vmswap) if (vmrss + vmswap) > 0 else 0)
...
def print_stats():
    ...
    if stats:
        print(f"Ready: {stats.total_ready_time // 1e6:.2f} ms, Blocked: {stats.total_blocked_time // 1e6:.2f} ms, Swapped: {stats.total_swapped_time // 1e6:.2f} ms, Running: {stats.total_running_time // 1e6:.2f} ms")
        total_time = stats.total_ready_time + stats.total_blocked_time + stats.total_swapped_time + stats.total_running_time
        elapsed_time = total_time
        if elapsed_time > 0:
            print(f"Total Observed Time: {total_time // 1e6:.2f} ms, Page Faults: {faults}, Faults/sec: {faults / elapsed_time:.2f}")
    else:
