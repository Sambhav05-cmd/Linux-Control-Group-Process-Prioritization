...
def second_child_fn(child_pid):
    ...
    while True:
        ...
        if stats:
            data1 = struct.pack("Q Q Q Q", int(stats.total_ready_time), int(stats.total_blocked_time),
                                int(stats.total_swapped_time), int(stats.total_running_time))
...
def print_stats():
    ...
    if stats:
        ...
        print(f"Ready: {stats.total_ready_time/total_time:.2f} ms, Blocked: {stats.total_blocked_time/total_time:.2f} ms, Swapped: {stats.total_swapped_time/total_time:.2f} ms, Running: {stats.total_running_time/total_time:.2f} ms")
        total_time = stats.total_ready_time + stats.total_blocked_time + stats.total_swapped_time + stats.total_running_time
        if elapsed_time > 0:
