...
TRACEPOINT_PROBE(sched, sched_switch) {
    ...
    if (*my_pid_ptr == prev_pid) {
        ...
        if (!s) {
            ...
            init.total_ready_time = 0;
    	    init.total_blocked_time = 0;
    	    init.total_swapped_time = 0;
    	    init.total_running_time = 0;
...
TRACEPOINT_PROBE(vmscan, mm_vmscan_lru_isolate) {
    ...
    struct PageEventStats *stats = pagefaults.lookup(&pid);
    if (stats) {
        stats->lru_isolate_count += args->nr_taken;
    } else {
        struct PageEventStats init = {};
        init.lru_isolate_count = args->nr_taken;
        init.fault_count = 0;
        init.swap_out_count = 0;
        pagefaults.update(&pid, &init);
    }
...
if pid == 0:
    ...
    # Move child to low_priority cgroup
    with open(cgroup_path, "w") as f:
        f.write(str(child_pid))

    SHM_SIZE = 1024  # enough for a few stats
    STATS_STRUCT = struct.Struct("d d d d Q Q Q d")  # ready, blocked, swapped, running, faults,resident page ratio
...
def second_child_fn(child_pid):
    ...
    while True:
        ...
        lru_isolate_count = mem_stats.lru_isolate_count if mem_stats else 0
        vmrss = 0
        vmswap = 0
...
def  print_stats():
    ...
    if stats:
        ...
        print(f"Swaps: {swaps}, Swaps/sec: {swaps / elapsed_time:.2f }")
        print(f"LRU isolat count : {lru_isolate_count}, /sec: {lru_isolate_count / elapsed_time:.2f }")
...
def prioritize_memory():
    ...
    high_mem_file = os.path.join(high_path, "memory.max")
    low_mem_file = os.path.join(low_path, "memory.max")
    parent_mem_file = os.path.join(parent_path, "memory.max")

    delta = 2 * 1024 * 1024  # 2 MB

    # Step 1: read current values
    with open(parent_mem_file, "r+") as pf:
        parent_val = pf.read().strip()
        if parent_val == "max":
            raise RuntimeError("Parent has no memory limit set")
        parent_val = int(parent_val)

    with open(high_mem_file, "r+") as hf:
        high_val = hf.read().strip()
        high_val = 0 if high_val == "max" else int(high_val)

    with open(low_mem_file, "r+") as lf:
        low_val = lf.read().strip()
        if low_val == "max":
            raise RuntimeError("low_priority has no memory limit set")
        low_val = int(low_val)
        if low_val < delta:
            raise RuntimeError("Not enough memory in low_priority to reduce")

    # Step 2: increase parent limit temporarily
    with open(parent_mem_file, "w") as pf:
        pf.write(str(parent_val + delta))

    # Step 3: increase high_priority
    with open(high_mem_file, "w") as hf:
        hf.write(str(high_val + delta))

    # Step 4: move process to high_priority
    with open(high_procs_file, "w") as f:
        f.write(str(child_pid))

    # Step 5: reduce low_priority
    with open(low_mem_file, "w") as lf:
        lf.write(str(low_val - delta))

    # Step 6: revert parent memory limit
    with open(parent_mem_file, "w") as pf:
...
while True:
    print("\n[p] Boost Priority  [o] reduce OOM score  [m] prioritize memory [s] Stats  [q] Quit")
    ch = input("> ").strip()
    if ch == 'p':
        boost_priority()
    elif ch == 'o':
